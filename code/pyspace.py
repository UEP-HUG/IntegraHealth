...
def compute_lisa_rate(df, col, e, b, w, n_permut, p_value, adjusted=True, transform_type='r'):
    li = Moran_Local_Rate(df[e], df[b], w, adjusted=adjusted, transformation=transform_type, permutations=n_permut, n_jobs=-1, seed = 42)
    sig = li.p_sim < p_value
    hotspot, lowhigh, coldspot, highlow = 1 * (sig * li.q == 1), 2 * (sig * li.q == 2), 3 * (sig * li.q == 3), 4 * (
                sig * li.q == 4)
    spots = hotspot + coldspot + lowhigh + highlow
...
def compute_getis(df, col, w, n_permut, transform_type='R', star=True, 
                 fdr=False, fdr_alpha=0.05, p_001=False, min_sig=0.1):
    """
    Compute Getis-Ord Gi statistic and classify results in a GeoDataFrame.
    ...
    transform_type : str, default 'R'
        Weights transformation type ('R' for row-standardized, 'B' for binary)
    star : bool, default True
        Whether to use Gi* (True) or Gi (False)
    fdr : bool, default False
        Apply False Discovery Rate correction
    fdr_alpha : float, default 0.05
        Alpha value for FDR correction
    p_001 : bool, default False
        Include 0.001 significance level
    min_sig : float, default 0.1
        Minimum significance threshold
        
    Returns
    -------
    G_Local
        The G_Local object with results
    """
    logging.info(f"Computing Getis-Ord Gi statistic for {col} with {n_permut} permutations")
    ...
    # Compute G statistic
    g = G_Local(df[col], w, transform=transform_type, n_jobs=-1, 
                star=star, seed=42, permutations=n_permut)
    ...
    # Apply classifications from smallest p-value to largest
...
def plot_getis_by_class(df, x, y, label, xtick_size=12, title_size=16, xlabel_size=12, ylabel_size=12, p_001=False,showfliers = False, boxplot=False):
    """
...
def plotGetisMap_ge(db, col, p_001=False,markersize_s=5, markersize_l=8, commune_name=False):
    """
    ...
    if commune_name:
        ...
        texts = communes.apply(lambda x: ax.annotate(text=x.NAME, 
    ...
    # Create a more sophisticated scale bar
    scalebar = ScaleBar(
        1, 
        units="m",
        dimension="si-length",
    ...
    try:
        ...
        print('Add north arrow from Cartopy')
        # Add north arrow using CartoPy's north arrow
        from cartopy.mpl.gridliner import LATITUDE_FORMATTER, LONGITUDE_FORMATTER
        
        # Add north arrow as a fancy arrow
        def add_north_arrow(ax, x, y, size=0.15, pad=0.1):
            ...
            # Add 'N' label
            ax.text(x, y+pad, 'N', transform=ax.transAxes,
                   ha='center', va='center', fontweight='bold', fontsize=12)
        
        # Add the north arrow to a good position
        add_north_arrow(ax, 0.90, 0.12, size=0.05, pad=0.02)
    except ImportError:
        # Method 2: Custom North Arrow implementation if CartoPy isn't available
        from matplotlib.patches import PathPatch
        from matplotlib.path import Path
        
        def add_fancy_north_arrow(ax, x, y, size=0.1, style='fancy'):
            """Add a fancy north arrow to the plot."""
            if style == 'simple':
                # Simple arrow pointing north
                ...
                # Fancy compass rose arrow
                vertices = [
                    (x, y+size),             # North point
                    (x+size/3, y-size/3),    # Right bottom
                    (x, y-size/6),           # Middle bottom inset
                    (x-size/3, y-size/3),    # Left bottom
                    (x, y+size),             # Back to North point
                ]
                codes = [Path.MOVETO] + [Path.LINETO] * 3 + [Path.CLOSEPOLY]
                path = Path(vertices, codes)
                arrow = PathPatch(path, facecolor='black', edgecolor='white', linewidth=1.5, 
        ...
        # Add the fancy north arrow
        add_fancy_north_arrow(ax, 0.90, 0.15, size=0.05)
    return fig, ax

def plotGetisMap_ge_simple(db, col,markersize_s=1, markersize_l=5, buffer=600, commune_name=False):
    """
    This function creates a Getis map and plots it using Matplotlib and GeoPandas.
    ...
    db = db.copy()
    print('Plot Getis Map')
    db.loc[db[col].str.contains('0.1'),col] = 'Not significant'
    # db.loc[db[col].str.contains('0.05'),col] = 'Not significant'
    db.loc[db[col].str.contains('Hot'),col] = 'Hot Spot - p < 0.05'
    db.loc[db[col].str.contains('Cold'),col] = 'Cold Spot - p < 0.05'
    ...
    colors_cl_getis = {
        'Cold Spot - p < 0.05': '#2166ac',
        'Hot Spot - p < 0.05': '#b2182b',
        'Not significant': '#ABABAB'
    }

    # Ordering colors according to the unique sorted values in the column
    hmap = colors.ListedColormap([colors_cl_getis[i] for i in db[col].sort_values().unique()])

    hot_spots = db[(db[col].str.contains('Hot'))]
    cold_spots = db[(db[col].str.contains('Cold'))]
    ns = db[(db[col].str.contains('Not'))]

    hot_spots['geometry'] = hot_spots['geometry'].buffer(buffer)
    hot_spots_dissolved = unary_union(hot_spots.geometry).simplify(5)

    cold_spots['geometry'] = cold_spots['geometry'].buffer(buffer)
    cold_spots_dissolved = unary_union(cold_spots.geometry).simplify(5)

    ns['geometry'] = ns['geometry'].buffer(buffer)
    ns_dissolved = unary_union(ns.geometry).simplify(5)
    ...
    lake.plot(ax=ax, label='Lake', alpha=1, color='lightgrey', zorder = 5)
    db.plot(y, cmap=hmap, markersize=0, linewidth = 0, ax=ax, legend=True, categorical=True, zorder=6)
    
    gpd.GeoSeries(hot_spots_dissolved).plot(color=colors_cl_getis['Hot Spot - p < 0.05'],edgecolor = 'grey', linewidth=1, alpha=0.7, zorder=7, ax=ax)
    gpd.GeoSeries(cold_spots_dissolved).plot(color=colors_cl_getis['Cold Spot - p < 0.05'] ,edgecolor = 'grey', linewidth=1, alpha=0.7, zorder=7, ax=ax)
    gpd.GeoSeries(ns_dissolved).plot(color=colors_cl_getis['Not significant'] ,edgecolor = 'grey', linewidth=1, alpha=0.4, zorder=6, ax=ax)

    legend = ax.get_legend()
    legend.set_bbox_to_anchor((0.15, 0.8, 0.2, 0.2))
    ax.set_axis_off()
    ax.set_facecolor('grey')
    ...
    # ax.add_artist(scalebar)
    # ax.set_axis_off()
    # x, y, arrow_length = 0.91, 0.15, 0.08
    # ax.annotate('N', xy=(x, y), xytext=(x, y - arrow_length),
    #             arrowprops=dict(facecolor='black', width=5, headwidth=15),
    #             ha='center', va='center', fontsize=20,
    #             xycoords=ax.transAxes)    
    ...
    try:
        ...
        # Add the north arrow to a good position
        add_north_arrow(ax, 0.91, 0.12, size=0.05, pad=0.02)
    except ImportError:
        # Method 2: Custom North Arrow implementation if CartoPy isn't available
        from matplotlib.patches import PathPatch
        from matplotlib.path import Path
        ...
        # Add the fancy north arrow
        add_fancy_north_arrow(ax, 0.91, 0.15, size=0.05)
    return fig, ax
...
def plotGetisMap(db, col, p_001=False,markersize_s=1, markersize_l=5, commune_name=False):
    ...
    cantons, communes = cantons.to_crs(2056), communes.to_crs(2056)
    cantons_to_map = cantons[['UUID', 'NAME', 'geometry']].sjoin(
    db[['geometry']], 
    predicate='contains', 
    how='inner').drop_duplicates(subset=['UUID'])[['UUID', 'NAME', 'geometry']]
...
def plotLISAMap_polygons(db, col, letter, commune_name=False, add_lakes = False):
    ...
    y = db[col] + ' [$n$=' + db[col].map(db.groupby(col)[col.split('_cl')[0]].size().astype(int)).astype(str) + ']'
    if add_lakes:
    ...
    box_color="white",
    location="lower right",
    pad=0.5,  # padding from the edge
    border_pad=0.5,  # padding between scalebar and box
    sep=5,  # separation between bar and text
    frameon=True,  # draw a frame around the scalebar
    scale_loc="bottom",  # location of scale text
    font_properties={"size": 10}  # control text size
    )
...
def plotLISA_points(db, col, letter, commune_name=False):
    """
    Visualize Getis results on a map
    ...
    Arguments
